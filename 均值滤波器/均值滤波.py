...
for x in range(padding_width, img_process.shape[0] - padding_width):
    for y in range(padding_width , img_process.shape[1] - padding_width):
        new_img[x - padding_width][y - padding_width] = int(np.sum(img_process[x - padding_width: x + padding_width + 1,y - padding_width: y + padding_width +1] * kernel) / kernel_size / kernel_size)


cv.imwrite("universe_process_kernelsize_{}.png".format(kernel_size), new_img)
